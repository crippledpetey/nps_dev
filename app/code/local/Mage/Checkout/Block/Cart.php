...
class Mage_Checkout_Block_Cart extends Mage_Checkout_Block_Cart_Abstract {
	...
	// overwrite the parent methos
	public function getItemHtml(Mage_Sales_Model_Quote_Item $item) {
		//prepare image info
		$image_path = 'http://images.needplumbingsupplies.com/catalog/product/';
		$image_size = '/80x80/';
		//default as placeholder
		$default_manu = 'needplumbingsupplies';
		$default_name = 'noimagenps.jpeg"';
		$new_src = $image_path . $default_manu . $image_size . $default_name;
		//check for db image
		$img_info = $this->_getImages($item->getProductId());
		if (!empty($img_info)) {
			//check to see if the image exists on server
			$test_src = $image_path . $img_info[0]['manu'] . $image_size . $img_info[0]['file_name'];
			if (@getimagesize($test_src)) {
				//set the image source with closing quotes
				$new_src = $test_src . '"';
			}
		}
		//get the html
		...
		$searchStart = 'src="';
		$searchEnd = '"';
		//get the character count for the START of the string
		$src_start = strpos($rendererHtml, $searchStart);
		//get the character count for the END of the string (offset to the start position plus the length of the string)
		$src_end = strpos($rendererHtml, $searchEnd, ($src_start + strlen($searchStart)));
		//get the string of the image src (run should be the (END - END length) - START )
		$old_src = substr($rendererHtml, ($src_start + strlen($searchStart)), (($src_end - strlen($searchEnd)) - $src_start));

		//replace the string
		$rendererHtml = str_replace($old_src, $new_src, $rendererHtml);
