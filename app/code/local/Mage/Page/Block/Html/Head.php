...
class Mage_Page_Block_Html_Head extends Mage_Core_Block_Template {
	...
	public function getDescription() {
		...
		$shortcodes = $npsSC->getShotcodeLocations($content);

		//if there are shortcodes process them
		if ($shortcodes) {
			//get the data
			$shortcodes = $npsSC->getShortcodeData($shortcodes, $content);
			//reset the description with the new shortcodes
			$content = $npsSC->processShortcodeData($shortcodes, $content, true);
		}

		//ouput description
		return htmlspecialchars(html_entity_decode(trim($content), ENT_QUOTES, 'UTF-8'));
	}
	...
	public function _getProductAttr($_product) {
		$attributeSetModel = Mage::getModel("eav/entity_attribute_set");
		$attributeSetModel->load($_product->getAttributeSetId());
		$attributeSetName = $attributeSetModel->getAttributeSetName();
		return $attributeSetName;
	}
	public function _getContainerProductID($product_id) {
		$query = "SELECT p.product_id FROM catalog_product_option AS p INNER JOIN catalog_product_option_type_value AS o ON o.option_id = p.option_id INNER JOIN catalog_product_entity AS e ON e.sku = o.sku WHERE e.entity_id = " . $product_id;
		return $this->sqlread->fetchRow($query);
	}
	public function _getChildOptionTypeID($entity_id) {
	...
	public function _getUrlOptionsForProduct($product_id) {
	...
	public function _getContainerProductURL($entity_id, $manual_get = null) {
		//get parents entity
		$parent_id = $this->_getContainerProductID($entity_id);
		...
		if (!empty($parent_id)) {
			...
			//write the url
			$url = $this->_writeProductURL($parent_id, $manual_get);
	...
	public function _writeProductURL($product_id, $manual_get = null) {
